class IntegrationSettingsController < ApplicationController
  ...
  def integration_setting_params
    params.require(:integration_setting).permit(:etl_strategy_name, :lead_strategy_name, :inventory_service_url, :inventory_service_auth_token, :lead_vendor_endpoint, :lead_vendor_user_name, :lead_vendor_user_name, :lead_vendor_password, :inventory_vendor_user_name, :inventory_vendor_password, :inventory_vendor_endpoint, custom_integration_settings_attributes: [:id, :name, :_destroy, :value])
  end
end